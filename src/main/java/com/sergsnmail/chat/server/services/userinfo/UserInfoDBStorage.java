...
public class UserInfoDBStorage extends DbStorage implements IUserInfoStorage{
    ...
    public UserInfo getUserInfo(String userName) {
        ...
        try(PreparedStatement preparedStatement = getConnection().prepareStatement(SELECT_USER_SQL)){
    ...
    public boolean updateUserInfo(UserInfo userInfo) {
        ...
        try (PreparedStatement preparedStatement = getConnection().prepareStatement(UPDATE_USER_SQL)) {
    ...
    public boolean createUserInfo(UserInfo userInfo, String hashedPassword) {
        ...
        try(PreparedStatement preparedStatement = getConnection().prepareStatement(INSERT_USER_SQL)){
    ...
    public boolean deleteUserInfo(UserInfo userInfo) {
        try(PreparedStatement preparedStatement = getConnection().prepareStatement(DELETE_USER_SQL)){
            preparedStatement.setString(1, userInfo.getUsername());
            preparedStatement.executeUpdate();
            return true;
        }catch (Exception e){
    ...
    protected void initStorage() {
        try {
            getConnection().createStatement().execute(CREATE_USER_TABLE_SQL);
